...
- fastembed_bge_m3      : BAAI/bge-m3（1024 维，#9 升级项，多语言/长文本更优；
                           切换后需删除 chroma_db/ 重启重建向量库）
...
def create_embeddings() -> Embeddings:
    ...
    if provider in ("fastembed_bge_zh", "fastembed_bge_m3"):
        try:
            from langchain_community.embeddings import FastEmbedEmbeddings

            model = "BAAI/bge-small-zh-v1.5" if provider == "fastembed_bge_zh" else "BAAI/bge-m3"
            logger.info("Embedding provider=fastembed(%s)", model)
