...
### 2026-09-08 | 会话记忆分层、混合工具编排与数据采集授权边界

- **现象/背景**：旧会话只在 `chat_sessions.history` 中保留最近 6 轮，助手报告又按 4000 字符截断；较长的经营追问会丢失已确认的门店、时间范围、用户纠正和待办。数据问题虽然有 `tools → intent` 图回边，但工具执行后实际直接进入分析，无法让模型依据观察结果进行下一步调用。另一个运行风险是美团/点评采集依赖 Windows Edge Cookie；离职或权限撤销后，不能继续以个人登录态刷新经营数据。
- **根因/修改**：
  1. 新增 `chat_messages` 作为完整、追加式的会话原文存储；`chat_sessions.history` 仅保留旧客户端兼容窗口。新增 `session_memories` 保存版本化结构化记忆（目标、实体、已确认事实及来源、决定、待办、用户纠正），原文不会因压缩删除；
  2. 每次请求按“结构化会话记忆 + token 预算内原文 + 当前问题”构造上下文。旧消息满足阈值后由当前 LLM 通道增量压缩为 JSON；摘要失败不推进覆盖序号，保留原文并在后续重试。`BIZ_CONVERSATION_*` 可控制最近原文轮数、上下文预算、压缩阈值与摘要大小；
  3. 意图分类保留关键词高确定性快路径；未命中、或知识词和数据词同时命中时，调用当前激活的 LLM 通道以 `IntentDecision` schema 复核，失败回退规则结果；
  4. 常规指标查询继续使用确定性 `tool_plan`；明确日期等规则无法安全解析的请求进入受限 ReAct。`tool_mode` / `tool_round` 使 LLM 可观察 ToolMessage 后继续决策，`BIZ_AGENT_MAX_TOOL_ROUNDS=3` 是硬上限，防止循环调用和失控消耗；
  5. 数据采集明确改为授权能力：在线刷新只允许使用公司授权的商家后台登录态；无授权时应停止刷新，继续使用数据库历史快照，或由在职数据负责人提供正式导出报表后导入。不得复制、导出或绕过任何个人/前公司的 Cookie 与账号。
- **影响范围**：`database/models.py`、`services/conversation_memory.py`、`api/chat.py`、`agent/intent.py`、`agent/nodes.py`、`agent/state.py`、`config/settings.py`；新增原文/摘要表会在现有 `init_db()` 建表流程中创建。
- **验证/决策**：会话记忆、意图、流式和路由聚焦测试共 `38 passed`；Python 语法检查通过。该验证不等同于真实公司登录态或实时美团数据验证，数据采集仅在具备授权账号或授权导出文件时启用。
