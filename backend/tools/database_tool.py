...
=================================================
- 真实模式：SQLAlchemy 查询 MySQL（日粒度聚合 + 品类/商品维度）
- Mock 模式：确定性模拟数据（1 号店近 7 天较前 7 天下滑 ~30%，与种子数据埋点一致）
...
def _mock_daily(store_id: int, start: date, end: date) -> list[dict]:
    """确定性模拟：1 号店近 7 天订单量 ×0.7、客单价略降；其余随机但可复现。"""
    today = date.today()
    base = 260 if store_id == 1 else random.Random(store_id * 7919).randint(130, 320)
    rng = random.Random(store_id * 7919 + start.toordinal())
    result = _daily_dates(start, end)
    for item in result:
        d = date.fromisoformat(item["date"])
        is_recent = store_id == 1 and 0 <= (today - d).days < 7
        factor = 0.70 if is_recent else 1.0
        orders = int(base * factor * rng.uniform(0.85, 1.15))
        aov = 31.0 if is_recent else 35.0  # 近 7 天客单价略降（凑单减少）
        item["orders"] = orders
        item["gmv"] = round(orders * aov, 2)
    return result
...
def get_sales_data(
    ...
    if settings.mock_mode:
        daily, prev_daily = _mock_daily(store_id or 1, start, end), _mock_daily(store_id or 1, prev_start, prev_end)
        top_products = [
            {"name": "炸鸡小吃拼盘", "category": "小吃", "orders": 812, "gmv": 14560.0},
            {"name": "招牌奶茶", "category": "饮料", "orders": 760, "gmv": 12160.0},
            {"name": "牛肉盖饭", "category": "主食", "orders": 430, "gmv": 11825.0},
            {"name": "提拉米苏", "category": "甜品", "orders": 360, "gmv": 7920.0},
            {"name": "美式咖啡", "category": "咖啡", "orders": 410, "gmv": 6150.0},
        ]
        # 含 prev_gmv，供归因工具计算品类贡献度（与种子数据埋点一致：小吃品类掉量）
        category_breakdown = [
            {"category": "饮料", "orders": 1900, "gmv": 38600.0, "prev_gmv": 39200.0},
            {"category": "小吃", "orders": 690, "gmv": 11980.0, "prev_gmv": 30100.0},
            {"category": "主食", "orders": 1050, "gmv": 26200.0, "prev_gmv": 27100.0},
            {"category": "甜品", "orders": 720, "gmv": 15050.0, "prev_gmv": 15600.0},
            {"category": "咖啡", "orders": 880, "gmv": 18100.0, "prev_gmv": 18250.0},
        ]
    else:
        ...
        if not daily and not settings.mock_mode:
            return {"success": False, "data": {}, "error": "未查询到数据：请确认数据库已初始化并运行种子脚本"}
...
def _query_real_campaigns(days: int = 30) -> dict:
    """
    优先读取真实推广数据（promotion_reports，智选展位下载）。
...
def get_campaign_data(
    ...
    数据源（real 模式）：优先 promotion_reports 真实推广报告（智选展位下载），
    为空时回退 campaigns 表。
    """
    if settings.mock_mode:
        campaigns = [
            {"id": 1, "name": "满减推广-抖音-01-1", "type": "满减", "channel": "抖音", "status": "running",
             "budget": 20000.0, "spent": 17600.0, "clicks": 9000, "conversions": 430, "roi": 1.21,
             "spent_ratio": 0.88, "budget_warn": True},
            {"id": 2, "name": "折扣券推广-美团-01-2", "type": "折扣券", "channel": "美团", "status": "running",
             "budget": 12000.0, "spent": 5200.0, "clicks": 3100, "conversions": 190, "roi": 2.10,
             "spent_ratio": 0.43, "budget_warn": False},
            {"id": 3, "name": "新客立减推广-小程序-01-3", "type": "新客立减", "channel": "小程序", "status": "planned",
             "budget": 8000.0, "spent": 0.0, "clicks": 0, "conversions": 0, "roi": None,
             "spent_ratio": 0.0, "budget_warn": False},
        ]
        data = {
            "store_id": store_id or 1,
            "campaigns": campaigns,
    ...
    # ---- real 模式：优先真实推广报告（promotion_reports） ----
