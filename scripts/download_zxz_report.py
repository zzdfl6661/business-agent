...
def main():
    ...
    with sync_playwright() as p:
        ...
        # 优先复用已打开的智选展位 data-report 页（多次刷新后 Edge 会有多个 tab，
        # pages[0] 可能是登录页/旧页面，按 URL 特征匹配更稳）
        page = None
        for ctx in browser.contexts:
            for pg in ctx.pages:
                if "peon-cpm-ncpm" in pg.url or "data-report" in pg.url:
                    page = pg
                    break
            if page:
                break

        frame = find_report_frame(page) if page else None
        if not frame:
            # 未找到 → 新开 tab 导航到智选展位数据报告页（登录态已持久化）
            ctx = browser.contexts[0] if browser.contexts else browser.new_context()
            page = page or ctx.new_page()
            print("[*] 未找到 data-report 页面，新开 tab 导航…")
            page.goto("https://e.dianping.com/app/peon-cpm-ncpm/html/data-report.html",
                      wait_until="domcontentloaded", timeout=45000)
            page.wait_for_timeout(5000)
            frame = find_report_frame(page)
        if not frame:
            print("[!] 无法定位 data-report iframe，请确认登录态有效（Edge 9222）")
            return
